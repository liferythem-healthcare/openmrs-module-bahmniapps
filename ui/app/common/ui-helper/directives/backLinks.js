     ...
     return {
         template: '<ul>' +
                        '<li ng-repeat="backLink in backLinks">' +
                            '<a class="back-btn" ng-if="backLink.action" accesskey="{{backLink.accessKey}}" ng-click="closeAllDialogs(); linkAction(homeBackLink.type, homeBackLink.value, homeBackLink.params)" id="{{backLink.id}}"> <span ng-bind-html="backLink.label"></span> </a>' +
